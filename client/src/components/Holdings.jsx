...
import {deleteHolding} from '../features/portfolio/portfolioSlice'
  ...
  return (
    <>
        <tr className=''>
            ...
            <td>{(holding.price_bought_at * holding.no_shares).toFixed(2)}</td>
            {holding.regularMarketPrice > holding.price_bought_at? (
              <td className='gain'>{(holding.regularMarketPrice * holding.no_shares).toFixed(2)}</td>
            ) : (
              <td className='loss'>{(holding.regularMarketPrice * holding.no_shares).toFixed(2)}</td>
            )}
            <td>
              <button className='btn btn-primary' onClick={() => dispatch(deleteHolding(holding._id))}>
